...
% psix = (AnalyticalSolution(p,x+p.x/2,y,z,mu,xi,eta,e)-AnalyticalSolution(p,x-p.dx/2,y,z,mu,xi,eta,e))/p.dx;
...
if p.testAnaCase == 4
    ...
    r = r + 1e-8;
%     psi = exp(-p.maxsigmaa*r);
    psi = -HG_sigmaa(p,e)*exp(-HG_sigmaa(p,e)*r).*(mu.*x+xi.*y+eta.*z)./r;    
end
